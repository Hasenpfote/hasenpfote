...
inline Quaternion::Quaternion(float s, const Vector3& v)
{
    w = s;
    x = v.x;
    y = v.y;
...
inline Quaternion::Quaternion(const std::array<float, 4>& q)
{
    w = q[0];
    x = q[1];
    y = q[2];
...
inline Quaternion& Quaternion::operator = (const std::array<float, 4>& q)
{
    w = q[0];
    x = q[1];
    y = q[2];
